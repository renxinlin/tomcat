 ...
 *              commonLoader
    ...
    private void initClassLoaders() {
        try {
            // 注意 commonLoader是打破了双亲委派机制 如果parent为null，底层调用SystemClassloader
    ...
    public void init() throws Exception {
        ...
        // 通过类加载器加载组件 通过反射加载Catalina从而实现解耦
    ...
    public static void main(String args[]) {
        ...
        synchronized (daemonLock) {
            if (daemon == null) {
                ...
                try {
                   // tomcat init  类加载器初始化 建立Catalina
                    bootstrap.init();
                } catch (Throwable t) {
                    handleThrowable(t);
                    t.printStackTrace();
        ...
        try {
            ...
            if (command.equals("startd")) {
                ...
                // 核心就是基于catalina 初始化server之后初始化各个组件
                daemon.load(args);
                // start
                daemon.start();
