...
public class Catalina {
    ...
    public void load() {
        ...
        // Create and execute our Digester 创建server.xml解析器
        ...
        try {
            try {
                // 加载server.xml文件 解析文件生成所有的组件对象
                // 一般server.xml都会存在，不存在则选择替代方案
            ...
            try {
                ...
                digester.push(this);
                // 解析server.xml 根据server.xml生成server实例对象
        ...
        try {
            // 初始化StandardServer组件 【这里注意 一般tomcat的玩法都是initInternal】（參見LifecycleBase.init()）
            // 并且通过server初始化旗下的service等组件
            // 参见StandardServer.initInternal
            getServer().init();
        } catch (LifecycleException e) {
