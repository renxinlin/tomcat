...
/**
 *
 * service 是server下的一个实例 是一个核心的tomcat容器
 * 其核心方法包含initInternal  以及startInternal
 */
public class StandardService extends LifecycleMBeanBase implements Service {
     ...
     */
    // service 的父组件Server 一个server可以有多个service
    ...
    // 容器的入口 engine 一个service多个连接器 一个容器 容器为责任链[pipeline+valve]形式
    ...
    // 容器 注意 tomcat的容器概念很多 server可以看做容器 container可以看做容器 engine也是容器 其管道上的组件可以看做容器
    // 但一般从内部代码来说 容器一般指Engine和其父类ContainerBase
    ...
    protected void startInternal() throws LifecycleException {
        ...
        if (engine != null) {
            // 但是start会调用相关engine下的子容器的相关startInternal
            // 以及实现initInternal的子容器【没实现的全部走ContainerBase的】
            synchronized (engine) {
                engine.start();
            }
        }
        // 并发线程工具初始化 注意这个Executor是tomcat的Executor
        synchronized (executors) {
            for (Executor executor: executors) {
                executor.start();
            }
            ...
            for (Connector connector: connectors) {
                try {
                    ...
                    if (connector.getState() != LifecycleState.FAILED) {
                        //  connector.start() 后调用 ProtocolHandler.start() 后调用 endpoint.start从而完成请求监听器的start
                        connector.start();
                    }
                } catch (Exception e) {
                    log.error(sm.getString(
    ...
    protected void initInternal() throws LifecycleException {
        // 注意service组件包含了connector和container[engine]
        super.initInternal();
        // 先初始化引擎 后初始化connector
        // StandardEngine 只是会初始化engine 不会初始化职责链上的其他组件init 但是start会调用相关的init
        if (engine != null) {
            engine.init();
        }

        /**
         * 在tomcat中每个组件都可以有自己的执行器线程池
         * 如果不配置则默认全局配置的线程池
         */
        ...
        //  mapperListener 主要是监听容器组件生码周期变化 从而crud mapper[请求监听和请求处理容器的映射]
        ...
        synchronized (connectorsLock) {
            for (Connector connector : connectors) {
                try {
                    // 初始化连接器 就是是socket相关组件
                    // 其初始化了protocolHandler以及EndPoint
                    connector.init();
                } catch (Exception e) {
