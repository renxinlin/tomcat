...
public abstract class ContainerBase extends LifecycleMBeanBase
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        Container children[] = findChildren();// 当前容器的子容器 engine-> host etc
        List<Future<Void>> results = new ArrayList<>();
        for (int i = 0; i < children.length; i++) {
            // 初始化各个子容器  engine->【 host - context - wp】  启动子节点
            // submit 执行 子容器的start方法 【child.start();】
        ...
        if (pipeline instanceof Lifecycle) {
            // 启动管道，管道的作用:类似netty的pipeline  里面的每种组件类似netty 的channel
            // tomcat的每一个组件都有一个阀门Valve 每一个组件的阀门如果存在相同的值，则请求继续向下传递  类似于责任链
            ((Lifecycle) pipeline).start();
        }

        // starting会激活监听器HostConfig.start方法 如果state為LifecycleState.NEW 则在start时会触发init方法
        // 这也是standardContext的initInternal被调用的方式
        setState(LifecycleState.STARTING);  // 会调用 fireLifecycleEvent(lifecycleEvent, data); 从而触发deployWar等操作

        // Start our thread
        // 后台任务启动处理【比如部署变更检测 心跳 realm后台任务处理等】
        threadStart();
    }
