...
public abstract class AbstractHttp11Protocol<S> extends AbstractProtocol<S> {
    ...
    /**
     * If true, the connectionUploadTimeout will be ignored and the regular socket timeout will be used for the full duration of the connection.
    ...
    /**
     * 调研一下gzip是不是只对 text/html text/xml text/plain有效
     * @return
     */
    @Override
    protected Processor createProcessor() {
        ...
        // 设置适配器 CoyoteAdapter 核心处理是service
        processor.setAdapter(getAdapter());
        // keepaive情况下  每个socket最大的连接个数
        processor.setMaxKeepAliveRequests(getMaxKeepAliveRequests());
        // 文件上传超时时间
        processor.setConnectionUploadTimeout(getConnectionUploadTimeout());
        // 是否忽略文件上传超时时间使用常规超时时间
        processor.setDisableUploadTimeout(getDisableUploadTimeout());
        //
        processor.setRestrictedUserAgents(getRestrictedUserAgents());

        // 这里设置默认的post请求的最大值 4*1024
